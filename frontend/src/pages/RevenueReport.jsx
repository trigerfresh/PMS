...
const RevenueReport = () => {
    ...
    const hotelAndBranchFilteredBookings = bookings.filter(b => {
        ...
        if (branchId) {
            const selectedHotelIds = hotels.filter(h => h.branch_id == branchId).map(h => Number(h.id));
    ...
    const bookingCounts = {
        ...
        cancelled_bookings: hotelAndBranchFilteredBookings.filter(b => String(b.status || '').toLowerCase().trim() === 'cancelled').length,
        current_bookings: hotelAndBranchFilteredBookings.filter(b => isTodayCheckIn(b.check_in_date)).length,
        reserved_bookings: hotelAndBranchFilteredBookings.filter(b => String(b.status || '').toLowerCase().trim() === 'reserved').length,
        checkedout_bookings: hotelAndBranchFilteredBookings.filter(b => String(b.status || '').toLowerCase().trim() === 'checkedout').length,
        about_to_checkout_bookings: hotelAndBranchFilteredBookings.filter(b => isAboutToCheckout(b.check_out_date, b.status)).length,
        checkout_overdue_bookings: hotelAndBranchFilteredBookings.filter(b => getCheckoutStatus(b.check_out_date, b.status) === 'overdue').length,
    }
